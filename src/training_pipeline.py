...
class TrainingPipeline:
    ...
    def save_models(self):
        """Save and register best model in Hopsworks"""
        logger.info("Saving and registering models...")
        ...
        if self.best_model_name == "neural_network":
            local_path = "models/AQI_NN"
            self.best_model["model"].save(local_path, include_optimizer=False)
            joblib.dump(self.best_model["scaler"], os.path.join(local_path, "scaler.pkl"))

            tf_meta = mr.tensorflow.create_model(
                name="AQI_NN",
                metrics=self.metrics["neural_network"],
                description="Neural Network for AQI prediction"
            )
            tf_meta.save(local_path)
            logger.info("✅ TensorFlow model registered successfully")

        elif self.best_model_name == "random_forest":
            local_path = "models/AQI_RF"
            os.makedirs(local_path, exist_ok=True)
            joblib.dump(self.best_model, os.path.join(local_path, "model.pkl"))

            skl_model = mr.sklearn.create_model(
                name="AQI_RF",
                metrics=self.metrics["random_forest"],
                description="Random Forest model for AQI prediction"
            )
            skl_model.save(local_path)
            logger.info("✅ Random Forest model registered successfully")

        elif self.best_model_name == "ridge":
            local_path = "models/AQI_Ridge"
            os.makedirs(local_path, exist_ok=True)
            joblib.dump(self.best_model, os.path.join(local_path, "model.pkl"))

            skl_model = mr.sklearn.create_model(
                name="AQI_Ridge",
                metrics=self.metrics["ridge"],
                description="Ridge Regression model for AQI prediction"
            )
            skl_model.save(local_path)
            logger.info("✅ Ridge model registered successfully")
